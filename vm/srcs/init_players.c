...
/*   Updated: 2017/10/07 21:26:55 by mnunnari         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			init_players(t_vm *vm)
{
	...
	while (i < vm->players_nbr)
	{
		...
		if (read(player.fd, buf, CHAMP_MAX_SIZE) != player.size)
			ft_err(1, "Error: %s: Header size not congruent", player.filename);
		set_buf(vm, (MEM_SIZE / vm->players_nbr) * i, buf, CHAMP_MAX_SIZE);
		close(player.fd);
		process = new_process(player.number,
				(MEM_SIZE / vm->players_nbr) * i, 0);
		ft_lstaddnew(&vm->processes, process, 0);
